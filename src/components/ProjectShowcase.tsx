...
function Image({ project }: { project: Project }) { return <a className={`project-image project-image-${project.number}`} aria-label={`Open ${project.name} project`} href={project.url} target="_blank" rel="noopener noreferrer"><span className="project-art" aria-hidden="true"><img src={`/images/project-${project.number}.png`} alt="" loading="lazy" /><span>{project.number}</span></span><span className="image-cta">OPEN ↗</span></a>; }
